...
int load_lpd_label(char *label_name)
{
  ...
  if (init == -1) {
    int ret = 0;
    ret     = loadLabelName(label_name, lpd_labels, OBJ_CLASS_NUM);
    if (ret < 0) {
      return -1;
    }
  }
...
int load_lpr_label(char *label_name)
{
  ...
  if (init == -1) {
    int ret = 0;
    ret     = loadLabelName(label_name, lpr_labels, STR_CLASS_NUM);
    if (ret < 0) {
      return -1;
    }
  }
...
int load_lpc_label(char *label_name)
{
  ...
  if (init == -1) {
    int ret = 0;
    ret     = loadLabelName(label_name, lpc_labels, COLOR_CLASS_NUM);
    if (ret < 0) {
      return -1;
    }
  }

...
int create(int im_height, int im_width, int im_channel, char *model_path)
{
    ...
    for (int i = 0; i < n_output; ++i) {
        LOGE("output_attrs[i].n_elems[%d] = %d\n", i, output_attrs[i].n_elems);
...
int yolo_post_process(char *grid0_buf, char *grid1_buf, char *grid2_buf,
                      int *ids, float *scores, float *boxes, float *points) {
    int ret;
    if(!created) {
...
//        LOGE("%s @ (%d %d %d %d) %f\n", det_result->name, det_result->box.left, det_result->box.top,det_result->box.right, det_result->box.bottom, det_result->prop);
        ...
        *(boxes+4*i+3) = detect_result_group.results[i].box.bottom;
        ...
        for (int j = 0; j < KEY_POINT_NUM; j++)
        {
            ...
            {
                points[(i * 17 + j) * 2 + 0] = -10000.0f;
                points[(i * 17 + j) * 2 + 1] = -10000.0f;
                continue;
            }
            points[(i * 17 + j) * 2 + 0] = det_result->point[j].x;
            points[(i * 17 + j) * 2 + 1] = det_result->point[j].y;
        }
    }

...
int main(int argc, char** argv)
{
  	...
  	for (int i = 0; i < detect_result_group.count; i++) {
		...
		int point_1_x = det_result->point.point_1_x;
		int point_1_y = det_result->point.point_1_y;
		int point_2_x = det_result->point.point_2_x;
		int point_2_y = det_result->point.point_2_y;
		int point_3_x = det_result->point.point_3_x;
		int point_3_y = det_result->point.point_3_y;
		int point_4_x = det_result->point.point_4_x;
		int point_4_y = det_result->point.point_4_y;
		int point_5_x = det_result->point.point_5_x;
		int point_5_y = det_result->point.point_5_y;
		
		rectangle(orig_img, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(255, 0, 0, 255), 1);
		putText(orig_img, text, cv::Point(x1, y1 + 12), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 0));
		circle(orig_img, cv::Point(point_1_x, point_1_y), 2, cv::Scalar(255, 0, 0), -1);
		circle(orig_img, cv::Point(point_2_x, point_2_y), 2, cv::Scalar(0, 255, 0), -1);
		circle(orig_img, cv::Point(point_3_x, point_3_y), 2, cv::Scalar(0, 0, 255), -1);
		circle(orig_img, cv::Point(point_4_x, point_4_y), 2, cv::Scalar(0, 255, 255), -1);
		circle(orig_img, cv::Point(point_5_x, point_5_y), 2, cv::Scalar(255, 0, 255), -1);
  	}

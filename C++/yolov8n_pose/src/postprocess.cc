...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "./data/class.txt"
                   ...
                   int32_t zp, float scale)
{
  ...
  for (int i = 0; i < grid_h; i++) {
    for (int j = 0; j < grid_w; j++) {
      // printf("%d %d %d %d\n", grid_h, grid_w, i, j);
      int8_t prob = input[(i * grid_h + j) * PROP_BOX_SIZE];
      if (prob >= thres_i8) {
        int     offset = (i * grid_h + j) * PROP_BOX_SIZE + 1;
        float   box_x1 = (j + 0.5 - decode_box(&input[offset + 16 * 0], 16, zp, scale)) * (float)stride;
        float   box_y1 = (i + 0.5 - decode_box(&input[offset + 16 * 1], 16, zp, scale)) * (float)stride;
        ...
        // printf("%f %f %f %f %f\n", box_x1, box_y1, box_x2, box_y2, sigmoid(deqnt_affine_to_f32(prob, zp, scale)));
        ...
        {
        	float point_x = (deqnt_affine_to_f32(input[offset + 64 + 3 * k + 0], zp, scale) * 2 + j) * (float)stride;
        	float point_y = (deqnt_affine_to_f32(input[offset + 64 + 3 * k + 1], zp, scale) * 2 + i) * (float)stride;
        	float point_conf = sigmoid(deqnt_affine_to_f32(input[offset + 64 + 3 * k + 2], zp, scale));
        	points.push_back(point_x);
        	points.push_back(point_y);
        	points.push_back(point_conf);
        }
        
        objProbs.push_back(sigmoid(deqnt_affine_to_f32(prob, zp, scale)));
